...
<style>
	...
	<%
		...
		// searchResult, searchMain 에서 hosActiveNum='1'(승인) 상태만 해당 페이지 볼수있게 
		boolean hosActNumApp = dao.getHosActNumApprove(hosId);
	%>	
...
<%
	if (userId == null && hosActNumApp == false && admin == null) { // 세션 없을때 회원가입 요청하는 페이지 보여질 것.
%>		
			...
			<div style="width: 100%; display: flex; justify-content: center;">
				<section>
					<form action="searchResult.jsp">
						<input type="text" name="hosSearch" placeholder="병원명 검색(OO동물병원)" style="width: 550px; height: 75px; margin-top: 40px; border-radius: 10px;" />
			...
			<form> 
				...
				<table width=70%>
					...
					<tr>
						...
						<th colspan="4">회원가입(승인) 하시면 많은 정보를 보실수 있습니다.</th>
			...
			<div align="center">
				...
				%>
					...
					<tr>
						...
						<%-- (5-20) --%>
						<td><%=hosNumber-- %></td>
						<td><%=dto.getHosName() %></td>
						<td><%=dto.getHosNewAddress() %></td>
						<td><%=dto.getHosTel() %></td>
					</tr>	
				<% 
...
<%	
	} else { // session check 유효성 검사에서 userId,hosId,admin 값이 있을시(회원가입 한 유저일시) 보여질 페이지.
%>
		<%-- (4-1) search(검색)form 만들기. --%>
		<div align="center">
			<section>
