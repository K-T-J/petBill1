		...
		<div align="center">
			<section>
				<form action="searchMain.jsp">
					<input type="text" name="mainHosSearch" placeholder="병원명 검색" style="width: 550px; height: 75px; margin-top: 40px; border-radius: 10px;" />
...
<%
	if (userId == null && hosActNumApp == false && admin == null) { // 세션 없을때 회원가입 요청하는 페이지 보여질 것. ------------(21.07.17 추가)
%>
		...
		<div align="center">
			...
			%>
				...
				<tr>
					...
					<%-- (30-5) --%>
					<td><%=hosMainNumber-- %></td>
					<td><%=mainDto.getHosName() %></td>
					<td><%=mainDto.getHosNewAddress() %></td>
					<td><%=mainDto.getHosTel() %></td>
				</tr>	
			<% 
...
<%
	} else { // session check userId,hosId,admin이 회원 가입이 된상태에서 해당 페이지 보여질 페이지.
%>
		<%-- (29) if문으로 DB에 저장되어있는 병원 있을시/없을시 나누어 페이징 처리해주기. --%>
		<br />
		<br />
