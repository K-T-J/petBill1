...
public HosDTO getHospital(String hosTel) {
	...
	try {
		...
		if(rs.next()) {
			...
			//진료비 추가 21.07.20
			dto.setHosBasicVaccin(rs.getString("hosBasicVaccin"));
			dto.setHosNeuteringMan(rs.getString("hosNeuteringMan"));
			dto.setHosNeuteringWoman(rs.getString("hosNeuteringWoman"));
			dto.setHosHeartWorm(rs.getString("hosHeartWorm"));
	...
	try {
		conn = getConnection();
			...
			String sql = "insert into hospital(hosNo, hosId, hosPw, hosUserName, hosProfile, hosMobile, hosName, hosTel, hosTime, "
					+ "hosSiAddress, hosGuAddress, hosNewAddress, hosOldAddress, hosNum, hosNumPhoto, hosBio, "
					+ "hosBasicVaccin, hosNeuteringMan, hosNeuteringWoman, hosHeartWorm, hosActiveNum, hosReg) "
			...
			//진료비추가 21.07.20
			pstmt.setString(16, dto.getHosBasicVaccin());
			pstmt.setString(17, dto.getHosNeuteringMan());
			pstmt.setString(18, dto.getHosNeuteringWoman());
			pstmt.setString(19, dto.getHosHeartWorm());
...
//<!— 진료비 추가 21.07.20 —>
	public int hosMemberUpdate(HosDTO dto) {
		...
		try {
			conn = getConnection();
			if(dto.getHosProfile() == null && dto.getHosNumPhoto() == null){ // 둘다없다 
				String sql = "update hospital set "
						+ "hosUserName=?, hosMobile=?, "
						+ "hosName=?, hosTel=?, hosTime=?, "
						+ "hosSiAddress=?, hosGuAddress=?, hosNewAddress=?, hosOldAddress=?, "
						+ "hosNum=?, hosBio=?, hosBasicVaccin=?, hosNeuteringMan=?, hosNeuteringWoman=?, hosHeartWorm=?, "
						+ "hosReg=sysdate, hosActiveNum='1' "
						+ "where hosId=? ";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, dto.getHosUserName());
				pstmt.setString(2, dto.getHosMobile());
				pstmt.setString(3, dto.getHosName());
				pstmt.setString(4, dto.getHosTel());
				pstmt.setString(5, dto.getHosTime());
				pstmt.setString(6, dto.getHosSiAddress());	
				pstmt.setString(7, dto.getHosGuAddress());
				pstmt.setString(8, dto.getHosNewAddress());
				pstmt.setString(9, dto.getHosOldAddress());
				pstmt.setString(10, dto.getHosNum());
				pstmt.setString(11, dto.getHosBio());
				//진료비추가 id앞에 
				pstmt.setString(12, dto.getHosBasicVaccin());
				pstmt.setString(13, dto.getHosNeuteringMan());
				pstmt.setString(14, dto.getHosNeuteringWoman());
				pstmt.setString(15, dto.getHosHeartWorm());
				pstmt.setString(16, dto.getHosId());
				
				result = pstmt.executeUpdate();
				
				System.out.println("사진 둘다 없는 dao 업데이트결과 : "+ result);
				
			}else if(dto.getHosProfile() == null && dto.getHosNumPhoto() != null) { //없으면 얘 빼고 업데이트 
				String sql = "update hospital set "
						+ "hosUserName=?, hosMobile=?, "
						+ "hosName=?, hosTel=?, hosTime=?, "
						+ "hosSiAddress=?, hosGuAddress=?, hosNewAddress=?, hosOldAddress=?, "
						+ "hosNum=?, hosNumPhoto=?, hosBio=?, hosBasicVaccin=?, hosNeuteringMan=?, hosNeuteringWoman=?, hosHeartWorm=?, "
						+ "hosReg=sysdate, hosActiveNum='1' "
						+ "where hosId=? ";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, dto.getHosUserName());
				...
				pstmt.setString(9, dto.getHosOldAddress());
				pstmt.setString(10, dto.getHosNum());
				pstmt.setString(11, dto.getHosNumPhoto());
				pstmt.setString(12, dto.getHosBio());
				//진료비추가 id앞에 
				pstmt.setString(13, dto.getHosBasicVaccin());
				pstmt.setString(14, dto.getHosNeuteringMan());
				pstmt.setString(15, dto.getHosNeuteringWoman());
				pstmt.setString(16, dto.getHosHeartWorm());
				pstmt.setString(17, dto.getHosId());
						
				result = pstmt.executeUpdate();
				
				System.out.println("프로필사진 없는 dao 업데이트결과 : "+ result);
				...
				System.out.println("사업자등록증 없는 dao 업데이트결과 : "+ result);
				...
				//진료비추가 id 앞에 
				pstmt.setString(14, dto.getHosBasicVaccin());
				pstmt.setString(15, dto.getHosNeuteringMan());
				pstmt.setString(16, dto.getHosNeuteringWoman());
				pstmt.setString(17, dto.getHosHeartWorm());
				pstmt.setString(18, dto.getHosId());
	...
	//병원회원 비밀번호 수정 (세션으로비교)
    public int pwupdateHos(String hosId, String hosPw, String hosPwModify) {
       int result = 0;
       Connection conn = null;
       PreparedStatement pstmt = null;
       ResultSet rs = null;
       ...
       try {
          ...
          if(rs.next()) {
             ...
             if(dbpw.equals(hosPw)) {
                sql = "update hospital set hosPw=? where hosId=?";
                pstmt = conn.prepareStatement(sql);
                pstmt.setString(1, hosPwModify);
	...
	//#3. 병원고유번호 받아서 해당 병원정보 불러오기
			public HosDTO getHosArticle(int hosNo) {
				...
				try {
					conn = getConnection();
					String sql = "select hosId, hosName, hosProfile, hosBio, hosSiAddress, hosGuAddress, hosNewAddress, hosTel, hosTime, "
					...
					if(rs.next()) {
						article = new HosDTO();
						article.setHosId(rs.getString("hosId"));
						article.setHosName(rs.getString("hosName"));
		...
		//후기 리스트 띄우기
			public void reviewList(int hosNo) {
				...
				try {
					conn = getConnection();
					String sql = "select hosName from hospital where hosNo=?";
					pstmt = conn.prepareStatement(sql);
					pstmt.setInt(1, hosNo);
					rs = pstmt.executeQuery();
					if(rs.next()) {
						hosDto = new HosDTO();
						hosDto.setHosName(rs.getString("hosName"));
						hosDto.setHosNo(rs.getInt("hosNo"));
					}
				...
				try {
					...
					String sql = "select B.*, r "
									+ "from (select A.*, rownum r "
										+ "from (select * from hospital where hosActiveNum = ? order by hosreg desc) A "
									+ "order by hosreg desc) B "
								+ "where r >= ? and r <= ?";
					pstmt = conn.prepareStatement(sql);
					pstmt.setInt(1, 2);
			...
			//#5. 병원로그인 - id찾기 병원명, 병원전화번호
			public String hosFindId(String hosName, String hosTel) {
				Connection conn = null;
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				String dbid = null;
				try {
					conn = getConnection();
					String sql = "select hosId from hospital where hosName=? and hosTel=?";
			...
			//#6. 병원로그인 - pw찾기 아이디, 이름, 휴대폰
			public String hosFindPw(String hosId, String hosUserName, String hosMobile) {
				Connection conn = null;
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				String dbpw = null;
				try {
					conn = getConnection();
					String sql = "select hosPw from hospital where hosId=? and hosUserName=? and hosMobile=?";
					pstmt=conn.prepareStatement(sql);
					
					pstmt.setString(1, hosId);
					pstmt.setString(2, hosUserName);
					pstmt.setString(3, hosMobile);
...
// -------------------------------------------------------------------------------------------(21.07.14 추가)--------------
			...
			public List getAdminHosArticles(int start, int end) {
			
			Connection conn = null;
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			List adminHosList = null;
			
			try {
				conn = getConnection();
				String sql = "select B.*, r "
								+ "from (select A.*, rownum r "
									+ "from (select * from hospital order by hosreg desc) A "
								+ "order by hosname) B "
							+ "where r >= ? and r <= ?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, start);
				pstmt.setInt(2, end);
				rs = pstmt.executeQuery();
				System.out.println("[HosDAO] adminHosList : " + rs);
				
				if (rs.next()) {
					adminHosList = new ArrayList();
					do {
						HosDTO dto = new HosDTO();
						...
						adminHosList.add(dto);
						
					} while (rs.next());
					System.out.println("[HosDAO] adminHosList : " + adminHosList);
				}
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
			...
			// 병원 검색 으로 대기 상태인 수 나타낼 메서드 --------------------------------------------------------------------(21.07.16 추가)
			public int getHosSearchJoinWaitingCount(String adminHosSearch) {
				Connection conn = null;
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				int waitingCount = 0;
				...
				try {
					conn = getConnection();
					String sql = "select count(*) from hospital where hosActiveNum = ? and hosName like '%" + adminHosSearch + "%'";
					pstmt = conn.prepareStatement(sql);
					pstmt.setInt(1, 2);
					rs = pstmt.executeQuery();
					if (rs.next()) {
			...
			// 병원 검색 으로 대기 상태인 수 List 나타낼 메서드 -------------------------------------------------------------------(21.07.16 추가)
			public List getHosSearchJoinWaitingList(int waitingStartRow, int waitingEndRow, String adminHosSearch) {
				Connection conn = null;
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				List hosJoinNumberList = null;
				...
				try {
					...
					String sql = "select B.*, r "
									+ "from (select A.*, rownum r "
										+ "from (select * from hospital where hosActiveNum = ? and hosName like '%" + adminHosSearch + "%') A "
									+ "order by hosreg) B "
								+ "where r >= ? and r <= ?";
					pstmt = conn.prepareStatement(sql);
					pstmt.setInt(1, 2);			
					pstmt.setInt(2, waitingStartRow);
					pstmt.setInt(3, waitingEndRow);
					rs = pstmt.executeQuery();
					if (rs.next()) {
						hosJoinNumberList = new ArrayList();
						do {
			...
			// 병원 검색으로 DB에 병원총 수 나타낼 메서드 -------------------------------------------------------------------(21.07.16 추가)
			public int getHosSearchArticleCount(String adminHosSearch) {
				Connection conn = null;
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				int count = 0;
				...
				try {
					conn = getConnection();
					String sql = "select count(*) from hospital where hosname like '%" + adminHosSearch + "%'";
					pstmt = conn.prepareStatement(sql);
					rs = pstmt.executeQuery();
					if (rs.next()) {
						count = rs.getInt(1);
			...
			// 병원 검색으로 DB에 병원 총 List 나타낼 메서드 -----------------------------------------------------------------(21.07.16 추가)
			public List getHosSearchArticlesList(int startRow, int endRow, String adminHosSearch) {
				Connection conn = null;
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				List adminHosList = null;
				...
				try {
					...
					String sql = "select B.*, r "
									+ "from (select A.*, rownum r "
										+ "from (select * from hospital where hosName like '%" + adminHosSearch + "%' order by hosname) A "
									+ "order by hosreg desc) B "
								+ "where r >= ? and r <= ?";
					pstmt = conn.prepareStatement(sql);
					pstmt.setInt(1, startRow);
			...
			// admin 병원정보 페이지에서 병원이름 클릭시 병원정보 보여질 메서드 -------------------------------------------------(21.07.16 추가)
			public HosDTO getAdminHos(int adminHosNo) {
				Connection conn = null; 
				PreparedStatement pstmt = null; 
				ResultSet rs = null;
				HosDTO adHosDto = null;
				
				try {
					conn = getConnection();
					String sql = "select * from hospital where hosNo=?";
					pstmt = conn.prepareStatement(sql);
					pstmt.setInt(1, adminHosNo);
					rs = pstmt.executeQuery();
					
					if (rs.next()) {
						adHosDto = new HosDTO();
						
						adHosDto.setHosNo(rs.getInt("hosNo"));
						adHosDto.setHosId(rs.getString("hosId"));
						adHosDto.setHosPw(rs.getString("hosPw"));
						adHosDto.setHosUserName(rs.getString("hosUserName"));
						adHosDto.setHosProfile(rs.getString("hosProfile"));
						adHosDto.setHosMobile(rs.getString("hosMobile"));
						adHosDto.setHosName(rs.getString("hosName"));
						adHosDto.setHosTel(rs.getString("hosTel"));
						adHosDto.setHosSiAddress(rs.getString("hosSiAddress"));
						adHosDto.setHosGuAddress(rs.getString("hosGuAddress"));
						adHosDto.setHosNewAddress(rs.getString("hosNewAddress"));
						adHosDto.setHosOldAddress(rs.getString("hosOldAddress"));
						adHosDto.setHosNum(rs.getString("hosNum"));
						adHosDto.setHosNumPhoto(rs.getString("hosNumPhoto"));
						adHosDto.setHosBio(rs.getString("hosBio"));
						adHosDto.setHosReg(rs.getTimestamp("hosReg"));
						adHosDto.setHosActiveNum(rs.getInt("hosActiveNum"));
						adHosDto.setHosReason(rs.getString("hosReason"));
						adHosDto.setHosTime(rs.getString("hosTime"));
						adHosDto.setHosBasicVaccin(rs.getString("hosBasicVaccin"));
						adHosDto.setHosNeuteringMan(rs.getString("hosNeuteringMan"));
						adHosDto.setHosNeuteringWoman(rs.getString("hosNeuteringWoman"));
						adHosDto.setHosHeartWorm(rs.getString("hosHeartWorm"));
					}
				} catch (Exception e) {
					e.printStackTrace();
				} finally {
			...
			// admin 병원정보 페이지에서 병원이름 검색시 병원정보 보여질 병원 총 수 메서드 ------------------------------------------(21.07.16 추가)
			public int getHosTotalSearchListCount(String adminHosSearch) {
				Connection conn = null;
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				int count = 0;
			...
			// admin 병원정보 페이지에서 병원이름 검색시 병원정보 전체 List 보여질 메서드 ---------------------------------------------(21.07.16 추가)
			public List getAdminHosSearchList(int startRow, int endRow, String adminHosSearch) {
				Connection conn = null;
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				...
				try {
					...
					rs = pstmt.executeQuery();
					if (rs.next()) {
						adminHosTotalList = new ArrayList();
						do {
							HosDTO dto = new HosDTO();
				...
				return adminHosTotalList;
				
			}
			
			
			// admin 병원정보 페이지에서 병원이름 클릭시 병원정보 보여질 메서드 -----------------------------------------------(21.07.16 추가)
			public int getAdminHosTotalCount() {
				Connection conn = null; 
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				int count = 0;
			...
			// admin 병원정보 페이지에서 병원이름 클릭시 병원정보 보여질 메서드 -----------------------------------------------(21.07.16 추가)
			public List getgetAdminHosTotalList(int startRow, int endRow) {
				Connection conn = null;
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				List adminHosTotalList = null;
			...
			// admin 병원정보 adminHosResult 페이지에서 병원 활동상태 변경해주는 메서드 ----------------------------------------------(21.07.16 추가)
			public boolean updateHosActiveNum(HosDTO dto, int hosAnInt, String hosReason) {
				Connection conn = null; 
				PreparedStatement pstmt = null; 
				boolean result = false;
				int resultSet = 0;
				...
				try {
					conn = getConnection();
					String sql = "update hospital set hosActiveNum=?, hosReason=? where hosNo=?";
					pstmt = conn.prepareStatement(sql);
					pstmt.setInt(1, hosAnInt);
					pstmt.setString(2, hosReason);
					pstmt.setInt(3, dto.getHosNo());
					resultSet = pstmt.executeUpdate();
					System.out.println("[HosDAO] resultSet : " + resultSet);
					if (resultSet == 1) {
						result = true;
					}
					System.out.println("[HosDAO] 병원활동상태 : " + result);
			...
			// searchResult, searchMain 에서 hosActiveNum='1'(승인) 만 구 평균 및 병원 리스트 볼수 있게 session 메서드 -----------(21.07.17 추가) 
			public boolean getHosActNumApprove(String hosId) {
				Connection conn = null; 
				PreparedStatement pstmt = null; 
				ResultSet rs = null; 
				List articleList = null;
				HosDTO dto = null;
				boolean hosActNumApp = false;
				
				try {
					conn = getConnection();
					String sql = "select hosId from hospital where hosActiveNum=?";
					pstmt = conn.prepareStatement(sql);
					pstmt.setInt(1, 1);
					rs = pstmt.executeQuery(); 
					System.out.println("[HosDAO]hosActNumApp : " + rs);
					...
					if (rs.next()) {
						...
						do {
							...
							for (int i = 0; i < articleList.size(); i++) {
								if (hosId.equals(dto.getHosId())) {
									hosActNumApp = true;
								} 
								System.out.println("[HosDAO] true? : " + hosActNumApp);
								System.out.println("[HosDAO] hosId : " + hosId);
								System.out.println("[HosDAO] dto.getHosId()) : " + dto.getHosId());
							}
						} while (rs.next());
